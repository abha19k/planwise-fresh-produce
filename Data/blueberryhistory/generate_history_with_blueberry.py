...
# Demand knobs
FAMILY_BASE_QTY = {"Apple": 80.0, "Pear": 70.0, "Blueberry": 65.0, "": 75.0}
...
BASE_LIST_PRICE_FAMILY = {"Apple": 2.40, "Pear": 2.80, "Blueberry": 6.50}
HARVEST_WINDOW = {"Apple": (8, 11), "Pear": (8, 10)}  # Blueberry handled year-round
...
def seasonal_demand_factor(product_family: str, day_of_year: int) -> float:
    ...
    if fam.startswith("apple"):
        ...
        phase, amp = 240, 0.28
    elif fam.startswith("blueberry"):
        # Aggregate blueberry demand: stronger in spring/summer,
...
def harvest_level(family: str, month: int, rng: np.random.Generator) -> int:
    ...
    if fam == "Blueberry":
        # Aggregate blueberry supply is available year-round because
        # different producing regions harvest in different months.
        # Keep availability high, but allow modest monthly variation.
        monthly_base = {
            1: 3.6, 2: 3.5, 3: 3.7, 4: 4.0,
            5: 4.2, 6: 4.4, 7: 4.5, 8: 4.3,
            9: 4.1, 10: 4.0, 11: 3.8, 12: 3.7
        }
        base = monthly_base.get(month, 4.0)
    else:
...
def seasonal_price_multiplier(family: str, month: int) -> float:
    ...
    if family == "Blueberry":
        # Year-round supply, with modest price pressure during
        # high-volume European summer availability.
        return {
            1: 1.08, 2: 1.07, 3: 1.05, 4: 1.03,
            5: 1.00, 6: 0.96, 7: 0.94, 8: 0.95,
            9: 0.98, 10: 1.00, 11: 1.04, 12: 1.07
        }.get(month, 1.00)
    return 1.00
